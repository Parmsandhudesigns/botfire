chrome.extension.sendMessage({action: "isRecordingOn"}, function(response) {
	if(response.action == 'true'){
		...
		$(function(){		
			var	SIZE_SETTING = size
			var	IS_CHECKOUT_SETTING = 0;
			
			if(location.href.indexOf('product/model') != -1) {
				//remove check error
				location.href="javascript:function validateProduct(){return ''}";
				var result_size=false;
				var p=document.getElementById("pdp_sizes");
				var q=p.getElementsByTagName("a");
				for(var i=0;i<q.length;i++){
					if(q[i].innerHTML.trim()===SIZE_SETTING){
						result_size=q[i].id;
						break;
					}
				}			
				if(result_size) {
					...
					var interval = setInterval(function() {
						if($('#miniAddToCart_header').text().indexOf('Item added to cart:') < 0) {
							...
							if(IS_CHECKOUT_SETTING == 1) {
								$('#miniAddToCart_actions').find('img').each(function(i, img) {
									if(img.alt != 'Checkout')
										return;
									$(img).trigger('click');
								});
	...
	function ddoAddToCart(id) {
		location.href="javascript:$('"+id+"').trigger('click'); addToCart(); void 0";
	}
});
